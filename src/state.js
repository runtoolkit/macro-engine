 ...
 *
 * Equivalent of macro:state/* + macro:flag/* + macro:player var system
 *
 * Entities are arbitrary string keys (player name, uuid, etc.).
 */
...
export class State {
  ...
  get(entity, key, defaultValue = undefined) {
    return this.#entity(entity).get(key) ?? defaultValue;
  }

  has(entity, key) { return this.#entity(entity).has(key); }

  delete(entity, key) { this.#entity(entity).delete(key); }

  clearAll(entity) { this.#store.delete(entity); }

  list(entity) { return [...this.#entity(entity).keys()]; }

  listAll() { return [...this.#store.keys()]; }

  // ── numeric helpers ───────────────────────────────────────────

  increment(entity, key, by = 1) {
    this.set(entity, key, (this.get(entity, key, 0)) + by);
  }

  decrement(entity, key, by = 1) { this.increment(entity, key, -by); }

  addDefault(entity, key, value) {
    if (!this.has(entity, key)) this.set(entity, key, value);
  }
  ...
  copy(srcEntity, srcKey, dstEntity, dstKey) {
    this.set(dstEntity, dstKey, this.get(srcEntity, srcKey));
  }

  // ── boolean flags (toggle/set/unset) ─────────────────────────
  ...
  /** is(entity, key, value) → true if state matches */
